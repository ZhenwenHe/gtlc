...
class BPlusTree ：public Object <T,TRAITS,ALLOC> {
{
public:
...
public:
    ...
    typedef BPlusTree<key_type, data_type, value_type, key_compare,
...
public:
    ...
    public:
        ...
        /// The key type of the BPlusTree. Returned by key().
        typedef typename BPlusTree::key_type                key_type;

        /// The data type of the BPlusTree. Returned by data().
        typedef typename BPlusTree::data_type               data_type;

        /// The value type of the BPlusTree. Returned by operator*().
        typedef typename BPlusTree::value_type              value_type;

        /// The pair type of the BPlusTree.
        typedef typename BPlusTree::pair_type               pair_type;
    ...
    private:
        ...
        typename BPlusTree::leaf_node*      currnode;
	...
	/// Also friendly to the base BPlusTree class, because erase_iter() needs
	/// to read the currnode and currslot values directly.
	friend class BPlusTree<key_type, data_type, value_type, key_compare, traits, allow_duplicates>;
    ...
    public:
        ...
        inline iterator(typename BPlusTree::leaf_node *l, unsigned short s)
    ...
    private:
        ...
        /// The currently referenced leaf node of the tree
        const typename BPlusTree::leaf_node*        currnode;
    ...
    public:
        ...
        inline const_iterator(const typename BPlusTree::leaf_node *l, unsigned short s)
            : currnode(l), currslot(s)
        { }
    ...
    public:
        ...
        inline reverse_iterator(typename BPlusTree::leaf_node *l, unsigned short s)
            : currnode(l), currslot(s)
        { }
    ...
    public:
        ...
        inline const_reverse_iterator(const typename BPlusTree::leaf_node *l, unsigned short s)
            : currnode(l), currslot(s)
        { }
...
public:
    ...
    /// comparison function
    explicit inline BPlusTree(const allocator_type &alloc = allocator_type())
    ...
    /// comparison object
    explicit inline BPlusTree(const key_compare &kcf,
    ...
    inline BPlusTree(InputIterator first, InputIterator last,
                 const allocator_type &alloc = allocator_type())
        : root(NULL), headleaf(NULL), tailleaf(NULL), allocator(alloc) 
    {
        insert(first, last);
    ...
    template <class InputIterator>
    inline BPlusTree(InputIterator first, InputIterator last, const key_compare &kcf,
                 const allocator_type &alloc = allocator_type())
        : root(NULL), headleaf(NULL), tailleaf(NULL),
          key_less(kcf), allocator(alloc),enable_shifting(false)
    {
        insert(first, last);
    }
...
public:
    ...
    protected:
        ...
        /// Constructor called from BPlusTree::value_comp()
        inline value_compare(key_compare kc)
            : key_comp(kc)
        { }

        /// Friendly to the BPlusTree class so it may call the constructor
        friend class BPlusTree<key_type, data_type, value_type, key_compare, traits, allow_duplicates>;
...
protected:
    ...
    inline int find_lower(const node_type *n, const key_type& key) const
    {
        ...
        BTREE_PRINT("BPlusTree::find_lower: on " << n << " key " << key << " -> (" << lo << ") " << hi << ", ");
    ...
    inline int find_upper(const node_type *n, const key_type& key) const
    {
        ...
        BTREE_PRINT("BPlusTree::find_upper: on " << n << " key " << key << " -> (" << lo << ") " << hi << ", ");
        ...
        if (selfverify)
        {
            ...
            BTREE_PRINT("BPlusTree::find_upper testfind: " << i << std::endl);
            BTREE_ASSERT(i == hi);
        }
        else {
            BTREE_PRINT(std::endl);
...
public:
    ...
    inline BPlusTree(const btree_self &other)
        : root(NULL), headleaf(NULL), tailleaf(NULL),
          stats( other.stats ),
          key_less( other.key_comp() ),
          allocator( other.get_allocator() )
...
private:
	...
	//shift the data in the leaf to its left sibling node 
	bool left_shift(leaf_node * leaf){
		//1.get the left sibling leaf node 
		leaf_node * left_sibling = leaf->prevleaf;
		//2. if left sibling is not full
		if(left_sibling)
		{
			if(!left_sibling->isfull()){
				inner_node * parent = find_parent_node(leaf);

				if(parent == find_parent_node(left_sibling))
				{
					// set the last key in the slotkey of the parent
					// be the most left key in the slotkey of the leaf;
					parent->slotkey[parent->slotuse-1] = leaf->slotkey[0];
					//shift the most left data to the sibling node
					left_sibling->slotkey[left_sibling->slotuse]=leaf->slotkey[0];
					left_sibling->slotdata[left_sibling->slotuse]=leaf->slotdata[0];
					left_sibling->slotuse++;
					leaf->slotuse--;
					//shift in the leaf
	...
	inner_node * find_parent_node(leaf_node * leaf){
		node *n = root;
		if (!n || leaf->slotuse<=0) return 0;
		key_type key=leaf->slotkey[0];
		inner_node *inner=0;
		while(!n->isleafnode())
		{
			inner = static_cast<inner_node*>(n);
			int slot = find_lower(inner, key);
			n = inner->childid[slot];
		}		
		return inner;
    ...
    */
                                             ...
                                             key_type* splitkey, node** splitnode)
    {
        if (!n->isleafnode())
        {
            ...
            BTREE_PRINT("BPlusTree::insert_descend into " << inner->childid[slot] << std::endl);
            ...
            if (newchild)
            {
                BTREE_PRINT("BPlusTree::insert_descend newchild with key " << newkey << " node " << newchild << " at slot " << slot << std::endl);

                if (inner->isfull())
                {
                    split_inner_node(inner, splitkey, splitnode, slot);

                    BTREE_PRINT("BPlusTree::insert_descend done split_inner: putslot: " << slot << " putkey: " << newkey << " upkey: " << *splitkey << std::endl);
                    ...
                    BTREE_PRINT("BPlusTree::insert_descend switch: " << slot << " > " << inner->slotuse+1 << std::endl);
                    ...
                    else if (slot >= inner->slotuse+1)
                    {
                        ...
                        BTREE_PRINT("BPlusTree::insert_descend switching to splitted node " << inner << " slot " << slot <<std::endl);
                    }
                }
        ...
        else // n->isleafnode() == true
        {
            ...
            if (leaf->isfull())
            {
				if(!left_shift(leaf))//是否能够通过兄弟节点直接中的平移，如果不能则分裂节点
				{
    ...
    void split_leaf_node(leaf_node* leaf, key_type* _newkey, node** _newleaf)
    {
        ...
        BTREE_PRINT("BPlusTree::split_leaf_node on " << leaf << std::endl);
    ...
    void split_inner_node(inner_node* inner, key_type* _newkey, node** _newinner, unsigned int addslot)
    {
        ...
        BTREE_PRINT("BPlusTree::split_inner: mid " << mid << " addslot " << addslot << std::endl);
        ...
        BTREE_PRINT("BPlusTree::split_inner_node on " << inner << " into two nodes " << mid << " and " << inner->slotuse - (mid + 1) << " sized" << std::endl);
...
public:
    ...
    {
        BTREE_PRINT("BPlusTree::erase_one(" << key << ") on BPlusTree size " << size() << std::endl);
    ...
    {
        BTREE_PRINT("BPlusTree::erase_iter(" << iter.currnode << "," << iter.currslot << ") on BPlusTree size " << size() << std::endl);
...
private:
    ...
    struct dump_header
    {
        /// "stx-BPlusTree", just to stop the restore() function from loading garbage
        char            signature[12];
...
public:
    ...
    bool restore(std::istream &is)
    {
        ...
        {
            BTREE_PRINT("BPlusTree::restore: file header does not match instantiation signature." << std::endl);
            return false;
        }
