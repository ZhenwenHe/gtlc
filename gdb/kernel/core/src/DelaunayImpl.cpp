...
#define REAL double
	...
	void triangulate(char *, struct CT3D2DDelaunayio *, struct CT3D2DDelaunayio *, struct CT3D2DDelaunayio *);
	void trifree(int *memptr);

	//获得T类型点集的法线Newill's algorithm 同数学库中的GetPointsNormalEx
	template <typename T>
	bool GetRingNormal(vector<T> & verts, Vertex3d& normal)
	{
		int nPtNum = verts.size();

		normal = Vertex3d(0, 0, 0);

		T vert = verts[nPtNum - 1];
		for (int i = 0; i<nPtNum; i++)
		{
			normal.x += (vert.z + verts[i].z) * (vert.y - verts[i].y);
			normal.y += (vert.x + verts[i].x) * (vert.z - verts[i].z);
			normal.z += (vert.y + verts[i].y) * (vert.x - verts[i].x);
			vert = verts[i];
		}
		normal.normalize();

		if (Vertex3d(0, 0, 0) == normal)
		{
			return false;
		}
	...
	bool GetLocalCoords(vector<T> & verts, Vertex3d normal, Vertex3d& ori, Vertex3d& ux, Vertex3d& uy, Vertex3d& uz)
	{
		...
		if (normal.z != 0.0)
		{
			ux.z = ori.z - ((ux.x - ori.x)*normal.x + (ux.y - ori.y)*normal.y) / normal.z;
		}
		else if (normal.x != 0.0)
		{
			ux.x = ori.x - ((ux.z - ori.z)*normal.z + (ux.y - ori.y)*normal.y) / normal.x;
		}
		else if (normal.y != 0.0)
		{
			ux.y = ori.y - ((ux.x - ori.x)*normal.x + (ux.z - ori.z)*normal.z) / normal.y;
		}
	...
	void MyInitWork(CT3D2DDelaunayio& in, CT3D2DDelaunayio& out)
	{
	...
	void MyFreeWork(CT3D2DDelaunayio& in, CT3D2DDelaunayio& out)
	{
		free(in.pointlist);
	...
	//重载函数用于对不同类型的顶点属性线性内插
	//其中pt为待内插点,ptstr ptend为原始顶点
	//插值属性包括z, u v, RGB
	//void InterpolationVertics(Vertex3f& pt,Vertex3d normal,Vertex3f& ptstr,Vertex3f& ptend);
	void InterpolationVertics(Vertex3d& pt, Vertex3d normal, Vertex3d& ptstr, Vertex3d& ptend);
	// void InterpolationVertics(TexturedVertex3f& pt,Vertex3d normal,TexturedVertex3f& ptstr,TexturedVertex3f& ptend);
	// void InterpolationVertics(TexturedVertex3d& pt,Vertex3d normal,TexturedVertex3d& ptstr,TexturedVertex3d& ptend);
	// void InterpolationVertics(ShadedVertex3f& pt,Vertex3d normal,ShadedVertex3f& ptstr,ShadedVertex3f& ptend);
	// void InterpolationVertics(ShadedVertex3d& pt,Vertex3d normal,ShadedVertex3d& ptstr,ShadedVertex3d& ptend);
	// void InterpolationVertics(VersatileVertex3f& pt,Vertex3d normal,VersatileVertex3f& ptstr,VersatileVertex3f& ptend);
	void InterpolationVertics(VersatileVertex3d& pt, Vertex3d normal, VersatileVertex3d& ptstr, VersatileVertex3d& ptend);
	...
	bool TriangulationFrmVerts(bool bIsClose, Vertex3d* normal, vector<int>* featurelines, vector<Vertex3d>* holemarks, vector<Vertex3d> &verts, TriangleMesh3dImpl* mesh)
	{
		...
		typedef vector<Vertex3d> _vectorVertex3d;
		_vectorVertex3d::iterator itr;
		vector<double> dcommonz;//z value
		dcommonz.resize(verts.size());
		_vectorVertex3d vertscopy(verts);//copy of verts

		//Get LCS
		itr = verts.begin();
		Vertex3d ori, ux, uy, uz;

		if (NULL == normal || normal->length2()<MathEx::TOL)
		{
			if (!GetRingNormal<Vertex3d>(verts, uz))
				return false;
		}
		else
		{
		...
		if (!GetLocalCoords<Vertex3d>(verts, uz, ori, ux, uy, uz))
			return false;

		//points
		CT3D2DDelaunayio in, out;
		...
		{
			Vertex3d vt = *itr;
			//convert from WCS to LCS
			ChangeXYZAxis(vt, ori, ux, uy, uz);
			//
			in.pointlist[lcount] = vt.x;
			in.pointlist[lcount + 1] = vt.y;
			dcommonz[lcount*0.5] = vt.z;
			//
			lcount += 2;
		}

		//featureline
		if (NULL == featurelines || 0 == featurelines->size())
		{
		...
		{
			in.numberofsegments = featurelines->size() / 2;
			in.segmentlist = (int *)malloc(in.numberofsegments * 2 * sizeof(int));

			vector<int>::iterator iitr;
			int i = 0;
			for (iitr = featurelines->begin(); iitr != featurelines->end(); iitr++)
			{
				in.segmentlist[i] = *iitr;
				i++;
			}
		}

		//处理重复点(map: redundancy point, origin point)
		typedef std::map<int, int> map_dupverts;
		map_dupverts dupVerts;
		vector<double>::iterator itr_zValue;
		_vectorVertex3d::iterator itr_vert;
		for (int i = 2 * (in.numberofpoints - 1); i >= 0; i -= 2)
		{
			for (int j = 0; j< i; j += 2)
			{
				if (fabs(in.pointlist[i] - in.pointlist[j])<MathEx::TOL&&fabs(in.pointlist[i + 1] - in.pointlist[j + 1])<MathEx::TOL)
				{
					dupVerts.insert(std::make_pair(i*0.5, j*0.5));
					//in.pointlist and outvertlist
					for (int k = i; k<2 * (in.numberofpoints - 1); k += 2)
					{
						in.pointlist[k] = in.pointlist[k + 2];
						in.pointlist[k + 1] = in.pointlist[k + 3];
					}
					in.numberofpoints--;
					//zValue
					itr_zValue = dcommonz.begin();
					itr_zValue += i*0.5;
					dcommonz.erase(itr_zValue);
					//copy of verts
					itr_vert = vertscopy.begin();
					itr_vert += i*0.5;
					vertscopy.erase(itr_vert);
					break;
				}
			}
		}
		...
		if (dupVerts.size()>0)
		{
			map_dupverts::iterator pos;
			//modify redundant index
			for (int j = 0; j<2 * in.numberofsegments; j++)
			{
				pos = dupVerts.find(in.segmentlist[j]);
				if (pos != dupVerts.end())
				{
					in.segmentlist[j] = pos->second;
				}
			}
			//modify consequent index
			for (int j = 0; j<2 * in.numberofsegments; j++)
			{
				int count = 0;
				for (pos = dupVerts.begin(); pos != dupVerts.end(); pos++)
				{
					if (in.segmentlist[j]>pos->first)
					{
						count++;
					}
					else
				...
				in.segmentlist[j] -= count;
			}
		}
		//hole
		if (NULL == holemarks || 0 == holemarks->size())
		{
			in.numberofholes = 0;
			in.holelist = NULL;
		}
		...
		{
			in.numberofholes = holemarks->size();
			in.holelist = (REAL *)malloc(in.numberofholes * 2 * sizeof(REAL));

			vector <Vertex3d>::iterator holeitr;
			int icount = 0;
			for (holeitr = holemarks->begin(); holeitr != holemarks->end(); holeitr++)
			{
				Vertex3d curvt = *holeitr;
				//convert from WCS to LCS
				ChangeXYZAxis(curvt, ori, ux, uy, uz);
				//
				in.holelist[icount] = curvt.x;
				in.holelist[icount + 1] = curvt.y;
				icount += 2;
			}
		}
		...
		if (bIsClose)
		{
			triangulate("zQp", &in, &out, NULL);
		}
		else
			triangulate("zQpc", &in, &out, NULL);

		//check
		if (out.trianglelist && 0 == out.numberoftriangles)
		{
			return false;
		}
		//
		Vertex3d* outvertlist = new Vertex3d[out.numberofpoints];
		if (out.numberofpoints == in.numberofpoints)
		{// no new vertices
			if (verts.size() == in.numberofpoints && 0 == dupVerts.size())
			{// exactly the original verts
				for (int i = 0; i<out.numberofpoints; i++)
				{
					outvertlist[i] = verts[i];
				}
			}
			else
			{// there exists redundancy
				if (verts.size() != out.numberofpoints + dupVerts.size())
				{
					return false;
				}
				int icount = out.numberofpoints - 1;
				map_dupverts::iterator pos;
		...
		{// gen new vertices
			...
			for (int i = in.numberofpoints * 2; i<out.numberofpoints * 2; i += 2)
			{
				...
				//
				//interpolate new point's zValue by original segment
				for (int i = 0; i<in.numberofsegments * 2; i += 2)
				{
					if (in.segmentlist[i] == in.segmentlist[i + 1])
					{
						continue;
					}
					...
					ptend.x = in.pointlist[in.segmentlist[i + 1] * 2];
					ptend.y = in.pointlist[in.segmentlist[i + 1] * 2 + 1];
				...
				ChangeXYZAxisReverse(curvt, ori, ux, uy, uz);
				outvertlist[i] = curvt;
			}
		}
		else
			return false;
		//
		mesh->SetVertices(outvertlist, out.numberofpoints);
		mesh->SetTriangles((long*)out.trianglelist, out.numberoftriangles * 3);
		mesh->SetVertexType(VERTEXTYPE_V3d);
		AABBoxImpl env;
		env.Union(outvertlist, out.numberofpoints);
		mesh->setEnvelope(env);
		//
		MyFreeWork(in, out);
	...
	bool TriangulationFrmVertsEx(bool bIsClose, Vertex3d* normal, vector<int>* featurelines, vector<Vertex3d>* holemarks, vector<VersatileVertex3d> &verts, unsigned char m_iVertexType, TriangleMesh3dImpl* mesh)
	{
		...
		typedef vector<VersatileVertex3d> _vectorVertex3d;
		_vectorVertex3d::iterator itr;
		...
		if (NULL == normal || normal->length2()<MathEx::TOL)
		{
			if (!GetRingNormal<VersatileVertex3d>(verts, uz))
				return false;
		}
		else
		{
		...
		if (!GetLocalCoords<VersatileVertex3d>(verts, uz, ori, ux, uy, uz))
			return false;
		...
		VersatileVertex3d* outvertlist = new VersatileVertex3d[out.numberofpoints];
		if (out.numberofpoints == in.numberofpoints)
		{// no new vertices
			if (verts.size() == in.numberofpoints && 0 == dupVerts.size())
			{// exactly the original verts
		...
		{// gen new vertices
			...
			for (int i = in.numberofpoints * 2; i<out.numberofpoints * 2; i += 2)
			{
				...
				{
					if (in.segmentlist[j] == in.segmentlist[j + 1])
					{
						continue;
					}
					...
					ptstr.z = dcommonz[in.segmentlist[j]];

					ptstr.u = vertscopy[in.segmentlist[j]].u;
					ptstr.v = vertscopy[in.segmentlist[j]].v;

					ptstr.cR = vertscopy[in.segmentlist[j]].cR;
					ptstr.cG = vertscopy[in.segmentlist[j]].cG;
					ptstr.cB = vertscopy[in.segmentlist[j]].cB;

					//end
					ptend.x = in.pointlist[in.segmentlist[j + 1] * 2];
					ptend.y = in.pointlist[in.segmentlist[j + 1] * 2 + 1];
					ptend.z = dcommonz[in.segmentlist[j + 1]];

					ptend.u = vertscopy[in.segmentlist[j + 1]].u;
					ptend.v = vertscopy[in.segmentlist[j + 1]].v;

					ptend.cR = vertscopy[in.segmentlist[j + 1]].cR;
					ptend.cG = vertscopy[in.segmentlist[j + 1]].cG;
		...
		//settomesh
		Vertex3d* evnVerts = new Vertex3d[out.numberofpoints];//used for 计算包围盒 0410
		if (m_iVertexType == VERTEXTYPE_V3f)
		{
			Vertex3f* newoutvertlist = new Vertex3f[out.numberofpoints];
			for (int i = 0; i<out.numberofpoints; i++)
			{
				newoutvertlist[i].x = outvertlist[i].x;
				newoutvertlist[i].y = outvertlist[i].y;
				newoutvertlist[i].z = outvertlist[i].z;

				evnVerts[i].x = outvertlist[i].x;
				evnVerts[i].y = outvertlist[i].y;
				evnVerts[i].z = outvertlist[i].z;
			}
			mesh->SetVertices(newoutvertlist, out.numberofpoints);
		}
		else if (m_iVertexType == VERTEXTYPE_V3d)
		{
			Vertex3d* newoutvertlist = new Vertex3d[out.numberofpoints];
			for (int i = 0; i<out.numberofpoints; i++)
			{
				newoutvertlist[i].x = outvertlist[i].x;
				newoutvertlist[i].y = outvertlist[i].y;
		...
		{
			TexturedVertex3f* newoutvertlist = new TexturedVertex3f[out.numberofpoints];
			for (int i = 0; i<out.numberofpoints; i++)
			{
				newoutvertlist[i].x = outvertlist[i].x;
				newoutvertlist[i].y = outvertlist[i].y;
				...
				newoutvertlist[i].u = outvertlist[i].u;
				newoutvertlist[i].v = outvertlist[i].v;
		...
		{
			TexturedVertex3d* newoutvertlist = new TexturedVertex3d[out.numberofpoints];
			for (int i = 0; i<out.numberofpoints; i++)
			{
				newoutvertlist[i].x = outvertlist[i].x;
				newoutvertlist[i].y = outvertlist[i].y;
		...
		{
			ShadedVertex3f* newoutvertlist = new ShadedVertex3f[out.numberofpoints];
			for (int i = 0; i<out.numberofpoints; i++)
			{
				newoutvertlist[i].x = outvertlist[i].x;
				newoutvertlist[i].y = outvertlist[i].y;
				...
				newoutvertlist[i].nx = outvertlist[i].nx;
				newoutvertlist[i].ny = outvertlist[i].ny;
				newoutvertlist[i].nz = outvertlist[i].nz;

				newoutvertlist[i].cR = outvertlist[i].cR;
				newoutvertlist[i].cG = outvertlist[i].cG;
				newoutvertlist[i].cB = outvertlist[i].cB;
		...
		{
			ShadedVertex3d* newoutvertlist = new ShadedVertex3d[out.numberofpoints];
			for (int i = 0; i<out.numberofpoints; i++)
			{
				newoutvertlist[i].x = outvertlist[i].x;
				newoutvertlist[i].y = outvertlist[i].y;
		...
		{
			VersatileVertex3f* newoutvertlist = new VersatileVertex3f[out.numberofpoints];
			for (int i = 0; i<out.numberofpoints; i++)
			{
				newoutvertlist[i].x = outvertlist[i].x;
				newoutvertlist[i].y = outvertlist[i].y;
		...
		{
			VersatileVertex3d* newoutvertlist = new VersatileVertex3d[out.numberofpoints];
			for (int i = 0; i<out.numberofpoints; i++)
			{
				newoutvertlist[i].x = outvertlist[i].x;
				newoutvertlist[i].y = outvertlist[i].y;
		...
		mesh->SetTriangles((long*)out.trianglelist, out.numberoftriangles * 3);
		mesh->SetVertexType(m_iVertexType);
		AABBoxImpl env;
		env.Union(evnVerts, out.numberofpoints);
		mesh->setEnvelope(env);
		delete[] evnVerts;
	...
	bool TriangulationFrmRings(bool bIsClose, vector<Vertex3d>* holemarks, vector<LinearRing3dImpl*>& rings, unsigned char iVertexType, TriangleMesh3dImpl* mesh, Vertex3d* vNormal)
	{
		if (NULL == mesh)
			return false;

		//计算Versertile点集与特征线集合
		typedef vector<VersatileVertex3d> _vectorVersatileVertex3d;
		typedef vector<int> _vectorInt;
		typedef vector<LinearRing3dImpl*> _vectorRing;
		//
		_vectorVersatileVertex3d verts;
		_vectorInt featurelines;
		_vectorRing::iterator ring_itr;
		Vertex3d normal;
		//
		int countVt = 0;
		for (ring_itr = rings.begin(); ring_itr != rings.end(); ring_itr++)
		{
			LinearRing3dImpl* curring = *ring_itr;
			countVt += curring->GetVerticesNum();
		}
		verts.resize(countVt);
		featurelines.resize(countVt * 2);
		//
		bool bIsGotNormal = false;
		if (vNormal && vNormal->length() != 0)
		{
			bIsGotNormal = true;
			normal = *vNormal;
		}
		...
		for (ring_itr = rings.begin(); ring_itr != rings.end(); ring_itr++)
		{
			LinearRing3dImpl* curring = *ring_itr;
			//
			eliminateRedundancy(curring);
			//normal
			if (!bIsGotNormal)
			{
				vector <Vertex3d> allverts;
				curring->GetVerticesEx(allverts);
				if (!GetRingNormal<Vertex3d>(allverts, normal))
					return false;
				bIsGotNormal = true;
			}
			//verts
			long vertNumber = 0;
			if (iVertexType == VERTEXTYPE_V3f)
			{
				Vertex3f* vt = NULL;
				curring->GetVertices((void*&)vt, vertNumber);
			...
			else if (iVertexType == VERTEXTYPE_V3d)
			{
				Vertex3d* vt = NULL;
				curring->GetVertices((void*&)vt, vertNumber);
				for (int i = 0; i<vertNumber; i++)
			...
			else if (iVertexType == VERTEXTYPE_V3fT2f)
			{
				TexturedVertex3f* vt = NULL;
				curring->GetVertices((void*&)vt, vertNumber);
				for (int i = 0; i<vertNumber; i++)
				{
					VersatileVertex3d curVt;
			...
			else if (iVertexType == VERTEXTYPE_V3dT2d)
			{
				TexturedVertex3d* vt = NULL;
				curring->GetVertices((void*&)vt, vertNumber);
				for (int i = 0; i<vertNumber; i++)
				{
					VersatileVertex3d curVt;
			...
			else if (iVertexType == VERTEXTYPE_V3fN3fC3f)
			{
				ShadedVertex3f* vt = NULL;
				curring->GetVertices((void*&)vt, vertNumber);
				for (int i = 0; i<vertNumber; i++)
				{
					VersatileVertex3d curVt;
			...
			else if (iVertexType == VERTEXTYPE_V3dN3dC3f)
			{
				ShadedVertex3d* vt = NULL;
				curring->GetVertices((void*&)vt, vertNumber);
				for (int i = 0; i<vertNumber; i++)
				{
					VersatileVertex3d curVt;
			...
			else if (iVertexType == VERTEXTYPE_V3fT2fN3fC3f)
			{
				VersatileVertex3f* vt = NULL;
				curring->GetVertices((void*&)vt, vertNumber);
				for (int i = 0; i<vertNumber; i++)
				{
					VersatileVertex3d curVt;
			...
			else if (iVertexType == VERTEXTYPE_V3dT2dN3dC3f)
			{
				VersatileVertex3d* vt = NULL;
				curring->GetVertices((void*&)vt, vertNumber);
				for (int i = 0; i<vertNumber; i++)
				{
					VersatileVertex3d curVt;
			...
			//featurelines ringIndex to ringIndex+vertNumer-1
			for (int i = ringIndex; i<ringIndex + vertNumber; i++)
			{
				if (i == ringIndex + vertNumber - 1)
				{
					featurelines[i * 2] = i;
					featurelines[i * 2 + 1] = ringIndex;
				}
				else
				{
					featurelines[i * 2] = i;
					featurelines[i * 2 + 1] = i + 1;
				}
			}
			ringIndex += vertNumber;
		}

		//构网
		return TriangulationFrmVertsEx(true, &normal, &featurelines, holemarks, verts, iVertexType, mesh);
	}
	/*
	template <typename T>
	bool TriangulationFrmRingsEx(bool bIsClose,vector<Vertex3d>* holemarks, vector<LinearRing3dImpl*>& rings,TriangleMesh3dImpl* mesh);

	bool TriangulationFrmRings (bool bIsClose,vector<Vertex3d>* holemarks, vector<LinearRing3dImpl*>& rings,unsigned char iVertexType,TriangleMesh3dImpl* mesh)
	{
	if(NULL == mesh)
	return false;
	...
	if (iVertexType == VERTEXTYPE_V3f)
	{
	TriangulationFrmRingsEx<Vertex3f>(bIsClose,holemarks,rings,mesh);
	}
	else if (iVertexType == VERTEXTYPE_V3d)
	{
	TriangulationFrmRingsEx<Vertex3d>(bIsClose,holemarks,rings,mesh);
	}
	else if (iVertexType == VERTEXTYPE_V3fT2f)
	{
	TriangulationFrmRingsEx<TexturedVertex3f>(bIsClose,holemarks,rings,mesh);
	}
	else if (iVertexType == VERTEXTYPE_V3dT2d)
	{
	TriangulationFrmRingsEx<TexturedVertex3d>(bIsClose,holemarks,rings,mesh);
	}
	else if (iVertexType == VERTEXTYPE_V3fN3fC3f)
	{
	TriangulationFrmRingsEx<ShadedVertex3f>(bIsClose,holemarks,rings,mesh);
	}
	else if (iVertexType == VERTEXTYPE_V3dN3dC3f)
	{
	TriangulationFrmRingsEx<ShadedVertex3d>(bIsClose,holemarks,rings,mesh);
	}
	else if (iVertexType == VERTEXTYPE_V3fT2fN3fC3f)
	{
	TriangulationFrmRingsEx<VersatileVertex3f>(bIsClose,holemarks,rings,mesh);
	}
	else if (iVertexType == VERTEXTYPE_V3dT2dN3dC3f)
	{
	TriangulationFrmRingsEx<VersatileVertex3d>(bIsClose,holemarks,rings,mesh);
	}
	mesh->SetVertexType(iVertexType);
	return true;
	}

	//模板函数实现多个类型环构网(没有赋顶点类型)
	template<typename T>
	bool TriangulationFrmRingsEx(bool bIsClose,vector<Vertex3d>* holemarks, vector<LinearRing3dImpl*>& rings,TriangleMesh3dImpl* mesh)
	{
	CT3D2DDelaunayio in, out;
	...
	typedef vector<LinearRing3dImpl*> _vectorRing;
	_vectorRing::iterator ring_itr;
	Vertex3d ori,ux,uy,uz;//LCS
	int lvertcount = 0;//total vertices number

	bool bIsgetLCS = false;
	for (ring_itr = rings.begin();ring_itr!= rings.end();ring_itr++)
	{
	LinearRing3dImpl* curring = *ring_itr;
	if (!bIsgetLCS)
	{
	vector<Vertex3d> vector_chkverts;
	curring->GetVerticesEx(vector_chkverts);

	if(GetRingNormal<Vertex3d>(vector_chkverts,uz))
	{
	if (GetLocalCoords<Vertex3d>(vector_chkverts,uz,ori,ux,uy,uz))
	bIsgetLCS = true;
	}
	}
	lvertcount += curring->GetVerticesNum();
	}
	if(!bIsgetLCS)
	{
	return false;
	...
	in.numberofpoints = lvertcount;
	in.pointlist = (REAL *) malloc(in.numberofpoints * 2 * sizeof(REAL));

	in.numberofsegments = lvertcount;// based on Rular

	in.segmentlist = (int *) malloc(in.numberofsegments * 2 * sizeof(int));

	//input points and segments
	int totalvert = 0;//itercount for vertex
	int totalvertin = 0;//itercount for vertex coords
	int totalsegment = 0;//itercount for segement
	int totalsegmentin = 0;//itercount for segement index
	double dcommonz = 0.0;// record the z value
	T* outvertlist = new T[in.numberofpoints];
	for (ring_itr = rings.begin();ring_itr!= rings.end();ring_itr++)
	{
	LinearRing3dImpl* curring = *ring_itr;
	//points
	long chkvtsnum = 0;
	T* chkvts;
	curring->GetVertices((void*&)chkvts,chkvtsnum);
	for (int i = 0;i< chkvtsnum;i++)
	{
	outvertlist[totalvert] = chkvts[i];
	//convert to LCS
	Vertex3d vt = Vertex3d(chkvts[i].x,chkvts[i].y,chkvts[i].z);
	ChangeXYZAxis(vt,ori,ux,uy,uz);
	//
	in.pointlist[totalvertin] = vt.x;
	in.pointlist[totalvertin+1] = vt.y;
	dcommonz = vt.z;
	//
	totalvert++;
	totalvertin+=2;
	...
	//segments
	for (int imark = totalsegment; imark < (chkvtsnum + totalsegment);imark++)
	{
	if (imark!=chkvtsnum + totalsegment-1)
	{
	in.segmentlist[totalsegmentin] = imark;
	in.segmentlist[totalsegmentin+1] = imark+1;
	}
	else
	{
	in.segmentlist[totalsegmentin] = imark;
	in.segmentlist[totalsegmentin+1] = totalsegment;
	}
	totalsegmentin+=2;
	}
	totalsegment+=chkvtsnum;

	_ASSERT(totalsegment==totalvert);
	}
	//处理重复点
	typedef std::map<int,int> map_dupverts;
	map_dupverts dupVerts;
	for(int i = 2*(lvertcount-1);i>0;i-=2)
	{
	for (int j = 0;j< i;j+=2)
	{
	if (fabs(in.pointlist[i]-in.pointlist[j])<MathEx::TOL&&fabs(in.pointlist[i+1]-in.pointlist[j+1])<MathEx::TOL)
	...
	dupVerts.insert(std::make_pair(i*0.5,j*0.5));
	//in.pointlist and outvertlist
	for (int k=i;k<2*(lvertcount-1);k+=2)
	{
	in.pointlist[k] = in.pointlist[k+2];
	in.pointlist[k+1] = in.pointlist[k+3];
	int index = k*0.5;
	outvertlist[index].x = outvertlist[index+1].x;
	outvertlist[index].y = outvertlist[index+1].y;
	outvertlist[index].z = outvertlist[index+1].z;
	}
	in.numberofpoints--;
	break;
	}
	...
	T* newoutvertlist = NULL;
	if (dupVerts.size()>0)
	{
	//pointlist
	newoutvertlist = new T[in.numberofpoints];
	memcpy(newoutvertlist,outvertlist,in.numberofpoints*sizeof(T));
	delete [] outvertlist;
	outvertlist=NULL;

	//segmentlist
	for (int j=0;j<2*lvertcount;j++)
	{
	map_dupverts::iterator pos,itr,itrb;
	pos = dupVerts.find(in.segmentlist[j]);
	if (pos!=dupVerts.end())
	{
	in.segmentlist[j] = pos->second;
	}
	itr = dupVerts.upper_bound(in.segmentlist[j]);
	if (itr!=dupVerts.begin())
	{
	int count = 0;
	itrb = dupVerts.begin();
	while (itrb!=itr)
	...
	vector<Vertex3d>::iterator hole_itr;
	int icount = 0;
	for (hole_itr = holemarks->begin(); hole_itr!= holemarks->end();hole_itr++)
	{
	Vertex3d curvt = *hole_itr;
	//convert from WCS to LCS
	ChangeXYZAxis(curvt,ori,ux,uy,uz);
	//
	in.holelist[icount] = curvt.x;
	...
	if (0 == out.numberoftriangles)
	{
	return false;
	}

	//如果生成了新顶点(ring折叠的情况 original from KML export)
	T* Oddnewoutvertlist = NULL;
	if(out.numberofpoints > in.numberofpoints)
	{
	Oddnewoutvertlist = new T[out.numberofpoints];
	int icount = 0;
	for (int i=0;i<out.numberofpoints;i++)
	{
	Vertex3d curvt;
	curvt.x = out.pointlist[icount];
	curvt.y = out.pointlist[icount+1];
	curvt.z = dcommonz;
	ChangeXYZAxisReverse(curvt,ori,ux,uy,uz);
	bool bIsFound = false;
	T foundVert;
	...
	if(NULL != outvertlist)
	{
	if(outvertlist[j].x == curvt.x && outvertlist[j].y == curvt.y && outvertlist[j].z == curvt.z)
	{
	foundVert = outvertlist[j];
	bIsFound = true;
	break;
	}
	}
	else if(NULL != newoutvertlist)
	{
	if(newoutvertlist[j].x == curvt.x && newoutvertlist[j].y == curvt.y && newoutvertlist[j].z == curvt.z)
	{
	foundVert = newoutvertlist[j];
	bIsFound = true;
	break;
	}
	}
	...
	{
	strpt = outvertlist[j];
	endpt = outvertlist[j+1];
	}

	Vertex3d testpt1, testpt2;
	testpt1.x = strpt.x;
	testpt1.y = strpt.y;
	testpt1.z = strpt.z;
	testpt2.x = endpt.x;
	testpt2.y = endpt.y;
	testpt2.z = endpt.z;
	if(1 == IsPointInLineSeg(curvt,testpt1,testpt2,NULL))
	{
	double dist1,dist2;
	dist1 = DPointToPoint(testpt1,curvt);
	dist2 = DPointToPoint(testpt1,testpt2);
	InterpolationVertics((T) Oddnewoutvertlist[i],(T) strpt,(T) endpt,dist1,dist2);
	break;
	}
	}
	}
	...
	if(j == in.numberofpoints-1)
	{
	strpt = newoutvertlist[j];
	endpt = newoutvertlist[0];
	}
	else
	{
	strpt = newoutvertlist[j];
	endpt = newoutvertlist[j+1];
	}
	Vertex3d testpt1, testpt2;
	testpt1.x = strpt.x;
	testpt1.y = strpt.y;
	...
	dist1 = DPointToPoint(testpt1,curvt);
	dist2 = DPointToPoint(testpt1,testpt2);
	InterpolationVertics(Oddnewoutvertlist[i],strpt,endpt,dist1,dist2);
	break;
	}
	}
	}
	Oddnewoutvertlist[i].x = foundVert.x;
	Oddnewoutvertlist[i].y = foundVert.y;
	Oddnewoutvertlist[i].z = foundVert.z;
	}
	icount+=2;
	}
	if(outvertlist)
	...
	mesh->SetVertices(outvertlist, lvertcount);
	Vertex3d* envVerts = new Vertex3d[lvertcount];
	for (int i=0;i<lvertcount;i++)
	{
	envVerts[i].x = outvertlist[i].x;
	envVerts[i].y = outvertlist[i].y;
	envVerts[i].z = outvertlist[i].z;
	}
	env.Union(envVerts,lvertcount);
	mesh->setEnvelope(env);
	delete [] envVerts;
	}
	else if (NULL != newoutvertlist)
	{
	mesh->SetVertices(newoutvertlist, out.numberofpoints);
	Vertex3d* envVerts = new Vertex3d[out.numberofpoints];
	for (int i=0;i<out.numberofpoints;i++)
	{
	envVerts[i].x = newoutvertlist[i].x;
	envVerts[i].y = newoutvertlist[i].y;
	envVerts[i].z = newoutvertlist[i].z;
	}
	env.Union(envVerts,out.numberofpoints);
	mesh->setEnvelope(env);
	delete [] envVerts;
	}
	else if(NULL != Oddnewoutvertlist)
	{
	mesh->SetVertices(Oddnewoutvertlist, out.numberofpoints);
	Vertex3d* envVerts = new Vertex3d[out.numberofpoints];
	for (int i=0;i<out.numberofpoints;i++)
	{
	envVerts[i].x = Oddnewoutvertlist[i].x;
	envVerts[i].y = Oddnewoutvertlist[i].y;
	envVerts[i].z = Oddnewoutvertlist[i].z;
	}
	env.Union(envVerts,out.numberofpoints);
	mesh->setEnvelope(env);
	delete [] envVerts;
	...
	#ifdef _DEBUG_MEMLEAK2_ZJQ
	long* tir = new long[out.numberoftriangles*3];
	for(int i = 0; i<out.numberoftriangles*3;i++)
	tir[i] = out.trianglelist[i];
	mesh->SetTriangles(tir, out.numberoftriangles*3);
	#else
	mesh->SetTriangles((long*)out.trianglelist, out.numberoftriangles*3);
	#endif // _DEBUG_MEMLEAK2_ZJQ
	...
	*/
	...
	void InterpolationVertics(Vertex3d& pt, Vertex3d normal, Vertex3d& ptstr, Vertex3d& ptend)
	{
		double dist_ptptstr = (pt.x - ptstr.x)*(pt.x - ptstr.x) + (pt.y - ptstr.y)*(pt.y - ptstr.y);
		double dist_ptstrptend = (ptstr.x - ptend.x)*(ptstr.x - ptend.x) + (ptstr.y - ptend.y)*(ptstr.y - ptend.y);
		double dratio = sqrt(dist_ptptstr / dist_ptstrptend);

		pt.z = ptstr.z + (ptend.z - ptstr.z) * dratio;
	}
	/*
	...
	pt.u = ptstr.u+(ptend.u-ptstr.u) * dratio;
	pt.v = ptstr.v+(ptend.v-ptstr.v) * dratio;
	}
	void InterpolationVertics(TexturedVertex3d& pt,Vertex3d normal, TexturedVertex3d& ptstr,TexturedVertex3d& ptend)
	{
	double dist_ptptstr = (pt.x - ptstr.x)*(pt.x - ptstr.x)+(pt.y - ptstr.y)*(pt.y - ptstr.y);
	...
	pt.cR = (float)(ptstr.cR * dratio + ptend.cR * (1-dratio));
	pt.cG = (float)(ptstr.cG * dratio + ptend.cG * (1-dratio));
	pt.cB = (float)(ptstr.cB * dratio + ptend.cB * (1-dratio));
	}
	void InterpolationVertics(ShadedVertex3d& pt,Vertex3d normal, ShadedVertex3d& ptstr,ShadedVertex3d& ptend)
	{
	double dist_ptptstr = (pt.x - ptstr.x)*(pt.x - ptstr.x)+(pt.y - ptstr.y)*(pt.y - ptstr.y);
	...
	*/
	...
	//////////////////////////////////////////////////////////////////////////
	void verySimpleTriangulation(LinearRing3dImpl* ring, TriangleMesh3dImpl*& mesh/*,bool bIsChk4*/);
	void TriangulationFrmSimpleFlatRingEx(vector<Vertex2d>& contour, long*& triangles, long& trianglenum);
	void fillVertsToMesh(LinearRing3dImpl* ring, TriangleMesh3dImpl* mesh);

	bool FastTriangulationFrmSimpleRing(LinearRing3dImpl* ring, unsigned char iVertexType, TriangleMesh3dImpl*& mesh, Vertex3d* normal)
	{
		if (NULL == ring)
		{
			return false;
		...
		//
		eliminateRedundancy(ring);

		//very fast triangulation
		if (ring->GetVerticesNum()<5)
		{
			verySimpleTriangulation(ring, mesh);
		}
		...
		if (NULL == mesh)
		{
			...
			ring->GetVerticesEx(chkvts);

			//Get LCS
			Vertex3d ori, ux, uy, uz;
			if (NULL == normal || normal->length2()<MathEx::TOL)
			{
				GetPointsNormalEx(chkvts, uz);
				if (uz.length2()<MathEx::TOL)
				{
					return false;
				}
			}
			...
			if (!GetLocalCoords<Vertex3d>(chkvts, uz, ori, ux, uy, uz))
				return false;

			//points
			vector<Vertex2d> verts;
			verts.resize(chkvts.size());
			int icount = 0;
			...
			//triangulation
			long* trianglelist = NULL;
			long trianglenum = 0;
			TriangulationFrmSimpleFlatRingEx(verts, trianglelist, trianglenum);
			if (NULL != trianglelist && 0 != trianglenum)
			{
				mesh = new TriangleMesh3dImpl;
				fillVertsToMesh(ring, mesh);
				//
				mesh->SetTriangles(trianglelist, trianglenum);
				mesh->SetVertexType(ring->GetVertexType());
			}
			else
			{
				return false;
		...
		if (NULL != mesh)
		{
			mesh->setEnvelope(ring->getEnvelope());
			return true;
		}
		else
			return false;
	...
	bool Snip(vector< Vertex2d > &contour, int u, int v, int w, int n, int *V)
	{
		int p;
		...
		//convex
		if (MathEx::TOL > (((Bx - Ax)*(Cy - Ay)) - ((By - Ay)*(Cx - Ax)))) return false;
	...
	void TriangulationFrmSimpleFlatRingEx(vector<Vertex2d>& contour, long*& triangles, long& trianglenum)
	{
		...
		//euler
		trianglenum = (n - 2) * 3;
		triangles = new long[trianglenum];
		//
		int *V = new int[n];
		...
		for (int m = 0, v = nv - 1; nv>2;)
		{
			...
			/* three consecutive vertices in current polygon, <u,v,w> */ //膜拜
			...
			if (Snip(contour, u, v, w, nv, V))
			{
				int s, t;
				/* output Triangle */
				triangles[tricount] = V[u];
				triangles[tricount + 1] = V[v];
				triangles[tricount + 2] = V[w];

				tricount += 3;
				m++;

				/* remove v from remaining polygon *///学习
	...
	void eliminateRedundantVerts(LinearRing3dImpl* ring, double TOL)
	{
		...
		T* verts = NULL;
		long vertNum = 0;
		ring->GetVertices((void*&)verts, vertNum);
		...
		//
		T* newverts = new T[vertNum - index.size()];
		int count = 0;
		...
		delete[] verts;
		verts = NULL;
		ring->SetVertices(count, newverts);
	}

	void eliminateRedundancy(LinearRing3dImpl* ring, double TOL)
	{
		if (ring->GetVertexType() == VERTEXTYPE_V3f)
		{
			eliminateRedundantVerts<Vertex3f>(ring, TOL);
		}
		else if (ring->GetVertexType() == VERTEXTYPE_V3d)
		{
			eliminateRedundantVerts<Vertex3d>(ring, TOL);
		}
		else if (ring->GetVertexType() == VERTEXTYPE_V3fT2f)
		{
			eliminateRedundantVerts<TexturedVertex3f>(ring, TOL);
		}
		else if (ring->GetVertexType() == VERTEXTYPE_V3dT2d)
		{
			eliminateRedundantVerts<TexturedVertex3d>(ring, TOL);
		}
		else if (ring->GetVertexType() == VERTEXTYPE_V3fN3fC3f)
		{
			eliminateRedundantVerts<ShadedVertex3f>(ring, TOL);
		}
		else if (ring->GetVertexType() == VERTEXTYPE_V3dN3dC3f)
		{
			eliminateRedundantVerts<ShadedVertex3d>(ring, TOL);
		}
		else if (ring->GetVertexType() == VERTEXTYPE_V3fT2fN3fC3f)
		{
			eliminateRedundantVerts<VersatileVertex3f>(ring, TOL);
		}
		else if (ring->GetVertexType() == VERTEXTYPE_V3dT2dN3dC3f)
		{
			eliminateRedundantVerts<VersatileVertex3d>(ring, TOL);
		}
	}

	//
	template <typename T>
	void fillVertsToMeshEx(LinearRing3dImpl* ring, TriangleMesh3dImpl* mesh)
	{
		T* vtlist = NULL;
		long vtNum = 0;
		ring->GetVertices((void*&)vtlist, vtNum);
		T* newvtlist = new T[vtNum];
		for (int i = 0; i<vtNum; i++)
		{
			newvtlist[i] = vtlist[i];
		}
		mesh->SetVertices(newvtlist, vtNum);
	}
	...
	void fillVertsToMesh(LinearRing3dImpl* ring, TriangleMesh3dImpl* mesh)
	{
		if (ring->GetVertexType() == VERTEXTYPE_V3f)
		{
			fillVertsToMeshEx<Vertex3f>(ring, mesh);
		}
		else if (ring->GetVertexType() == VERTEXTYPE_V3d)
		{
			fillVertsToMeshEx<Vertex3d>(ring, mesh);
		}
		else if (ring->GetVertexType() == VERTEXTYPE_V3fT2f)
		{
			fillVertsToMeshEx<TexturedVertex3f>(ring, mesh);
		}
		else if (ring->GetVertexType() == VERTEXTYPE_V3dT2d)
		{
			fillVertsToMeshEx<TexturedVertex3d>(ring, mesh);
		}
		else if (ring->GetVertexType() == VERTEXTYPE_V3fN3fC3f)
		{
			fillVertsToMeshEx<ShadedVertex3f>(ring, mesh);
		}
		else if (ring->GetVertexType() == VERTEXTYPE_V3dN3dC3f)
		{
			fillVertsToMeshEx<ShadedVertex3d>(ring, mesh);
		}
		else if (ring->GetVertexType() == VERTEXTYPE_V3fT2fN3fC3f)
		{
			fillVertsToMeshEx<VersatileVertex3f>(ring, mesh);
		}
		else if (ring->GetVertexType() == VERTEXTYPE_V3dT2dN3dC3f)
		{
			fillVertsToMeshEx<VersatileVertex3d>(ring, mesh);
		}
	}

	void verySimpleTriangulation(LinearRing3dImpl* ring, TriangleMesh3dImpl*& mesh)
	{
		typedef vector<Vertex3d> _vectorVertexInring;
		vector<Vertex3d> chkvts;

		if (ring->GetVerticesNum() == 3)
		{
			mesh = new TriangleMesh3dImpl;
			fillVertsToMesh(ring, mesh);
			//
			...
			tri[1] = 1;
			tri[2] = 2;
			mesh->SetTriangles(tri, 3);
			mesh->SetVertexType(ring->GetVertexType());
		}
		else if (ring->GetVerticesNum() == 4)
		{
			//
			mesh = new TriangleMesh3dImpl;
			fillVertsToMesh(ring, mesh);
			...
			tri[4] = 2;
			tri[5] = 3;
			mesh->SetTriangles(tri, 6);
			mesh->SetVertexType(ring->GetVertexType());
		}
	}

	/*****************************************************************************/
	/* For old c program the declaration of function can be just a function name*/
	/* without the parameters. define ANSI_DECLARARORS means using the modern c*/
...
#ifdef _DEBUG_MEMALLOC_ZJQ
		memptr = new int[(unsigned int)size / sizeof(int)];
#else
		memptr = (int *)malloc((unsigned int)size);
#endif // _DEBUG_MEMALLOC_ZJQ
...
#define STARTINDEX 1
...
#ifdef _DEBUG_MEMALLOC_ZJQ
		pool->firstblock = new int*[(pool->itemsfirstblock * pool->itembytes + (int) sizeof(int *) +
			pool->alignbytes) / sizeof(int)];
#else
		pool->firstblock = (int **)
...
#ifdef _DEBUG_MEMALLOC_ZJQ
					newblock = new int*[(pool->itemsperblock * pool->itembytes +
						(int) sizeof(int *) +
						pool->alignbytes) / sizeof(int)];
#else
...

	{
...
#ifdef _DEBUG_MEMALLOC_ZJQ
		m->dummytribase = (triangle *) new int*[(trianglebytes +
			m->triangles.alignbytes) / sizeof(int)];
#else
		...
		if (b->usesegments) {
			...
			*/
#ifdef _DEBUG_MEMALLOC_ZJQ
			m->dummysubbase = (subseg *)new int[(subsegbytes +
				m->subsegs.alignbytes) / sizeof(int)];
#else
...
#define Two_One_Product(a1, a0, b, x3, x2, x1, x0) \
...

	{
		...
		if (ahead < 0.0) {
			...
			if (((torg[0] < searchpoint[0]) == (searchpoint[0] < tdest[0])) &&
				((torg[1] < searchpoint[1]) == (searchpoint[1] < tdest[1]))) {//modified by john
				return ONEDGE;
			}
		}
...
#ifdef _DEBUG_MEMALLOC_ZJQ
		sortarray = (vertex *) new int[(m->invertices * (int) sizeof(vertex)) / sizeof(int)];
#else
...
#ifdef _DEBUG_MEMALLOC_ZJQ
			regiontris = (struct otri *) new int[(regions *
				(int) sizeof(struct otri)) / sizeof(int)];
#else
...
#ifdef _DEBUG_MEMALLOC_ZJQ
			*pointlist = (REAL *) new int[((int)(outvertices * 2 * sizeof(REAL))) / sizeof(int)];
#else
...
#ifdef _DEBUG_MEMALLOC_ZJQ
			*pointattriblist = (REAL *) new int[((int)(outvertices * m->nextras *
...
#ifdef _DEBUG_MEMALLOC_ZJQ
			*pointmarkerlist = (int *) new int[((int)(outvertices * sizeof(int))) / sizeof(int)];
#else
...
#ifdef _DEBUG_MEMALLOC_ZJQ
			*trianglelist = (int *) new int[((int)(m->triangles.items *
				((b->order + 1) * (b->order + 2) /
...
#ifdef _DEBUG_MEMALLOC_ZJQ
			*triangleattriblist = (REAL *) new int[((int)(m->triangles.items *
				m->eextras *
				sizeof(REAL))) / sizeof(int)];
#else
...
#ifdef _DEBUG_MEMALLOC_ZJQ
			*segmentlist = (int *) new int[((int)(m->subsegs.items * 2 *
...
#ifdef _DEBUG_MEMALLOC_ZJQ
			*segmentmarkerlist = (int *) new int[((int)(m->subsegs.items *
				sizeof(int))) / sizeof(int)];
#else
...
#ifdef _DEBUG_MEMALLOC_ZJQ
			*edgelist = (int *) new int[((int)(m->edges * 2 * sizeof(int))) / sizeof(int)];
#else
...
#ifdef _DEBUG_MEMALLOC_ZJQ
			*edgemarkerlist = (int *) new int[((int)(m->edges * sizeof(int))) / sizeof(int)];
#else
...
#ifdef _DEBUG_MEMALLOC_ZJQ
			*vpointlist = (REAL *) new int[((int)(m->triangles.items * 2 *
				sizeof(REAL))) / sizeof(int)];
#else
...
#ifdef _DEBUG_MEMALLOC_ZJQ
			*vpointattriblist = (REAL *)new int[((int)(m->triangles.items *
				m->nextras * sizeof(REAL))) / sizeof(int)];
#else
...
#ifdef _DEBUG_MEMALLOC_ZJQ
			*vedgelist = (int *) new int[((int)(m->edges * 2 * sizeof(int))) / sizeof(int)];
#else
...
#ifdef _DEBUG_MEMALLOC_ZJQ
			*vnormlist = (REAL *) new int[((int)(m->edges * 2 * sizeof(REAL))) / sizeof(int)];
#else
...
#ifdef _DEBUG_MEMALLOC_ZJQ
			*neighborlist = (int *) new int[((int)(m->triangles.items * 3 *
				sizeof(int))) / sizeof(int)];
#else
...
#ifdef ANSI_DECLARATORS
	void triangulate(char *triswitches, struct CT3D2DDelaunayio *in,
	struct CT3D2DDelaunayio *out, struct CT3D2DDelaunayio *vorout)
#else /* not ANSI_DECLARATORS */
	void triangulate(triswitches, in, out, vorout)
		char *triswitches;
	struct CT3D2DDelaunayio *in;
	struct CT3D2DDelaunayio *out;
	struct CT3D2DDelaunayio *vorout;
#endif /* not ANSI_DECLARATORS */
...

	{
		...
		triangleinit(&m);											//初始化内存结构
#ifdef TRILIBRARY
		parsecommandline(1, &triswitches, &b);
#else /* not TRILIBRARY */
		parsecommandline(argc, argv, &b);							//解析命令到behavior b中
#endif /* not TRILIBRARY */
		...
		if (vorout != (struct CT3D2DDelaunayio *) NULL) {
...
#ifndef CDT_ONLY
		if (m.regions > 0) {
			trifree((int *)regionarray);
		}
#endif /* not CDT_ONLY */
		if (m.holes > 0) {
			trifree((int *)holearray);
		}
		if (b.geomview) {
