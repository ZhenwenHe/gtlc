...
static char* GTLWin32Recode(const char* src,
...
void GTLClearRecodeStubWarningFlags()
{
	bHaveWarned1 = FALSE;
	bHaveWarned2 = FALSE;
	bHaveWarned3 = FALSE;
...
* </ul>
*
* If an error occurs an error may, or may not be posted with gtl::error().
*
...
*
* @return a NULL terminated string which should be freed with free().
*/

char *GTLRecodeStub(const char *pszSource,
...

{
	...
	if (pszSrcEncoding[0] == '\0')
		pszSrcEncoding = GTL_ENC_ISO8859_1;

	if (pszDstEncoding[0] == '\0')
		pszDstEncoding = GTL_ENC_ISO8859_1;

	/* -------------------------------------------------------------------- */
	/*      ISO8859 to UTF8                                                 */
	/* -------------------------------------------------------------------- */
	if (strcmp(pszSrcEncoding, GTL_ENC_ISO8859_1) == 0
		&& strcmp(pszDstEncoding, GTL_ENC_UTF8) == 0)
	{
		int nCharCount = strlen(pszSource);
		char *pszResult = (char *)calloc(1, nCharCount * 2 + 1);
	...
	/*      UTF8 to ISO8859                                                 */
	/* -------------------------------------------------------------------- */
	if (strcmp(pszSrcEncoding, GTL_ENC_UTF8) == 0
		&& strcmp(pszDstEncoding, GTL_ENC_ISO8859_1) == 0)
	{
		int nCharCount = strlen(pszSource);
		char *pszResult = (char *)calloc(1, nCharCount + 1);
		...
		&& strcmp(pszDstEncoding, GTL_ENC_UTF8) == 0)
	{
		...
		if (nCode > 0) {
			return GTLWin32Recode(pszSource, nCode, CP_UTF8);
		}
	}
		...
		&& strncmp(pszDstEncoding, "CP", 2) == 0)
	{
		...
		if (nCode > 0) {
			return GTLWin32Recode(pszSource, CP_UTF8, nCode);
		}
	}
#endif
	...
	/*      a one-time warning.                                             */
	/* -------------------------------------------------------------------- */
	if (strcmp(pszDstEncoding, GTL_ENC_UTF8) == 0)
	{
		int nCharCount = strlen(pszSource);
		char *pszResult = (char *)calloc(1, nCharCount * 2 + 1);
...
* free(), or NULL if an error occurs.
*/

char *GTLRecodeFromWCharStub(const wchar_t *pwszSource,
	const char *pszSrcEncoding,
	const char *pszDstEncoding)

{
	...
	if (strcmp(pszSrcEncoding, "WCHAR_T") != 0 &&
		strcmp(pszSrcEncoding, GTL_ENC_UTF8) != 0
		&& strcmp(pszSrcEncoding, GTL_ENC_UTF16) != 0
		&& strcmp(pszSrcEncoding, GTL_ENC_UCS2) != 0
		&& strcmp(pszSrcEncoding, GTL_ENC_UCS4) != 0)
	{
		gtl::error(gtl::ERRORTYPE::GET_FAILURE, gtl::ERRORCODE::GEC_APPDEFINED,
			"Stub recoding implementation does not support\n"
			"GTLRecodeFromWCharStub(...,%s,%s)",
			pszSrcEncoding, pszDstEncoding);
		return NULL;
	}
	...
	pszResult = (char *)malloc(nDstBufSize); // nearly worst case.
	...
	char *pszFinalResult =
		GTLRecodeStub(pszResult, GTL_ENC_UTF8, pszDstEncoding);
...
*
* @return the zero terminated wchar_t string (to be freed with free()) or
* NULL on error.
*
* @since GDAL 1.6.0
*/

wchar_t *GTLRecodeToWCharStub(const char *pszSource,
	const char *pszSrcEncoding,
	const char *pszDstEncoding)

{
	...
	if (strcmp(pszSrcEncoding, GTL_ENC_UTF8) != 0
		&& strcmp(pszSrcEncoding, GTL_ENC_ASCII) != 0)
	{
		pszUTF8Source = GTLRecodeStub(pszSource, pszSrcEncoding, GTL_ENC_UTF8);
	...
	if (strcmp(pszDstEncoding, "WCHAR_T") != 0
		&& strcmp(pszDstEncoding, GTL_ENC_UCS2) != 0
		&& strcmp(pszDstEncoding, GTL_ENC_UCS4) != 0
		&& strcmp(pszDstEncoding, GTL_ENC_UTF16) != 0)
	{
		gtl::error(gtl::ERRORTYPE::GET_FAILURE, gtl::ERRORCODE::GEC_APPDEFINED,
			"Stub recoding implementation does not support\n"
			"GTLRecodeToWCharStub(...,%s,%s)",
			pszSrcEncoding, pszDstEncoding);
		return NULL;
	}
	...
	wchar_t *pwszResult = (wchar_t *)calloc(sizeof(wchar_t), nSrcLen + 1);
...
* @since GDAL 1.7.0
*/
int GTLIsUTF8Stub(const char* pabyData, int nLen)
{
	if (nLen < 0)
...
char* GTLWin32Recode(const char* src, unsigned src_code_page, unsigned dst_code_page)
{
	...
	wchar_t* tbuf = (wchar_t*)calloc(sizeof(wchar_t), wlen + 1);
	tbuf[wlen] = 0;
	...
	/* Do the actual conversion */
	char* pszResult = (char*)calloc(sizeof(char), len + 1);
